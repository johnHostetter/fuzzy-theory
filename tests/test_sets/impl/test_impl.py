...
class TestFuzzySetImpl(unittest.TestCase):
    ...
    def test_no_op_sympy_formula_returns_none(self) -> None:
        """
        Coverage/regression test: NoOp.sympy_formula() (excluded from the sweeps
        above, since a no-op has no membership formula to plot) had no direct test
        coverage of its own confirming it returns None rather than raising.

        Returns:
            None
        """
        self.assertIsNone(NoOp.sympy_formula())
